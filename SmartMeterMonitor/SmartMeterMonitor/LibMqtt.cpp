...
static  time_t          tmSysStartTimestamp_l       =  0;       // Timestamp of System Start
...
int  iRes;
    ...
    tmSysStartTimestamp_l = time(NULL);                         // Timestamp of System Start (base to calculate Uptime for KeepAlive)
...
int            iRes;
    ...
    if (uiPayloadLen_p == 0)
    {
        TRACE0("    Hint: PayloadLen==0 -> Nothing to do.\n");
        return (0);
    }
...
int     iRes;
    ...
    if ((tmCurrTimestamp - tmLastKeepAliveTimestamp_l) >= tmKeepAliveInterval_l)
    {
        ...
        tmSysUptime = tmCurrTimestamp - tmSysStartTimestamp_l;

        snprintf(szKeepAliveMsg, sizeof(szKeepAliveMsg), "%lu", (unsigned long)tmSysUptime);
...
int            iRes;
    ...
    iOldSocket = MqttTransport_SetGetDataSocket(iSocket);
    iRes = MQTTPacket_read(abMqttRawDataPacketBuff, iBuffSize, MqttTransport_GetData);
    MqttTransport_SetGetDataSocket(iOldSocket);
    if (iRes != CONNACK)
    {
        TRACE0("FAILED!\n");
        return (-5);

/****************************************************************************
  ...
  Description:  Implementation for Eastron SDM630
...
SmartMeter::tError  SDM630::Setup (const char* pszRtuInterface_p, long lBaudrate_p, int iModbusSlaveID_p, const char* pszLabel_p /* = NULL */)
{

    TRACE2("SDM630: Create Modbus Interface ('%s', %dBd, 8N1)...\n", pszRtuInterface_p, lBaudrate_p);
    ...
    TRACE1("SDM630: m_pMBInst = 0x%04X\n", (unsigned long)m_pMBInst);

    TRACE1("SDM630: Set active Modbus Slave to ID=%u\n", iModbusSlaveID_p);
...
SmartMeter::tError  SDM630::ReadModbusRegs (void)
{
...
tError  ErrCode;
    ...
    TRACE0("SDM630: Connect to Modbus Interface...\n");
    ...
    TRACE0("SDM630: Flush Modbus Interface...\n");
    ...
    TRACE2("SDM630: Read RegisterBlock from Slave (RegStart=%d, RegCount=%d)...\n", MODBUS_REG_START, MODBUS_REG_COUNT);
    ...
    TRACE1("SDM630: Read %d registers\n", iNumOfRegs);


    TRACE0("SDM630: Disconnect from Modbus Interface...\n");
    modbus_close(m_pMBInst);

    TRACE0("SDM630: Modbus RTU released.\n");
    return (ErrCode);
...
const char*  SDM630::DumpModbusRegs (void)
{
...
int  iIdx;
    ...
    TRACE2("SDM630: DumpModbusRegs -> %d of %d byte buffer was used\n", strlen(m_szModbusRegDump), sizeof(m_szModbusRegDump));
...
void  SDM630::GetMeasurements (SDM630::tMeasurements* pMeasurements_p)
{
...
const char*  SDM630::GetMeasurementsAsText (void)
{
...
const char*  SDM630::GetMeasurementsAsJson (time_t tmTimeStamp_p /* = 0 */, bool fAddDevInfo_p /* = false */)
{
...
int  SDM630::GetModbusSlaveID (void)
{
...
const char*  SDM630::GetDevTypeName (void)
{
...
const char*  SDM630::GetErrorText (SmartMeter::tError SmError_p)
{
...
void  SDM630::DecodeMeasurements (SDM630::tMeasurements* pMeasurements_p)
{
    ...
    pMeasurements_p->m_flVoltage_L2  = modbus_get_float_abcd(&m_aui16ModbusRegBuff[2]);     // Reg  2/3:  Voltage L2                 [V]
    pMeasurements_p->m_flVoltage_L3  = modbus_get_float_abcd(&m_aui16ModbusRegBuff[4]);     // Reg  4/5:  Voltage L3                 [V]
    pMeasurements_p->m_flCurrent_L1  = modbus_get_float_abcd(&m_aui16ModbusRegBuff[6]);     // Reg  6/7:  Current L1                 [A]
    pMeasurements_p->m_flCurrent_L2  = modbus_get_float_abcd(&m_aui16ModbusRegBuff[8]);     // Reg  8/9:  Current L2                 [A]
    pMeasurements_p->m_flCurrent_L3  = modbus_get_float_abcd(&m_aui16ModbusRegBuff[10]);    // Reg 10/11: Current L3                 [A]
    pMeasurements_p->m_flPower_L1    = modbus_get_float_abcd(&m_aui16ModbusRegBuff[12]);    // Reg 12/13: Power L1                   [W]
    pMeasurements_p->m_flPower_L2    = modbus_get_float_abcd(&m_aui16ModbusRegBuff[14]);    // Reg 14/15: Power L2                   [W]
    pMeasurements_p->m_flPower_L3    = modbus_get_float_abcd(&m_aui16ModbusRegBuff[16]);    // Reg 16/17: Power L3                   [W]
    pMeasurements_p->m_flCurrent_Sum = modbus_get_float_abcd(&m_aui16ModbusRegBuff[48]);    // Reg 48/49: Current Sum                [A]
    pMeasurements_p->m_flPower_Sum   = modbus_get_float_abcd(&m_aui16ModbusRegBuff[52]);    // Reg 52/53: Total system power         [W]
...
const char*  SDM630::BuildTextString (void)
{

SDM630::tMeasurements  Measurements;
    ...
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Voltage_L2  (Reg  2/3 ):\t%.1f [V]\n",   Measurements.m_flVoltage_L2);
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Voltage_L3  (Reg  4/5 ):\t%.1f [V]\n",   Measurements.m_flVoltage_L3);
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Current_L1  (Reg  6/7 ):\t%.3f [A]\n",   Measurements.m_flCurrent_L1);
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Current_L2  (Reg  8/9 ):\t%.3f [A]\n",   Measurements.m_flCurrent_L2);
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Current_L3  (Reg 10/11):\t%.3f [A]\n",   Measurements.m_flCurrent_L3);
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Power_L1    (Reg 12/13):\t%.3f [W]\n",   Measurements.m_flPower_L1);
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Power_L2    (Reg 14/15):\t%.3f [W]\n",   Measurements.m_flPower_L2);
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Power_L3    (Reg 16/17):\t%.3f [W]\n",   Measurements.m_flPower_L3);
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Current_Sum (Reg 48/49):\t%.3f [A]\n",   Measurements.m_flCurrent_Sum);
    LibSys::JoinStr(m_szMeasurementsText, sizeof(m_szMeasurementsText), "Power_Sum   (Reg 52/53):\t%.3f [W]\n",   Measurements.m_flPower_Sum);
    ...
    TRACE2("SDM630: BuildTextString -> %d of %d byte buffer was used\n", strlen(m_szMeasurementsText), sizeof(m_szMeasurementsText));
...
const char*  SDM630::BuildJsonString (time_t tmTimeStamp_p, bool fAddDevInfo_p)
{
...
char                   szTimeStamp[64];
    ...
    TRACE2("SDM630: BuildJsonString -> %d of %d byte buffer was used\n", strlen(m_szMeasurementsJson), sizeof(m_szMeasurementsJson));

/****************************************************************************

  Copyright (c) 2026 Ronald Sieber

  Project:      SmartMeterMonitor (Read Measurements using Modbus/RTU)
  Description:  Declarations for Janitza UMG96RM

  -------------------------------------------------------------------------

  When implementing this class, top priority was given to robust and
  reliable 24/7 continuous operation. For this reason, types such as
  std::string, which work with dynamic memory management at runtime,
  were not used. Static buffers are used instead.

  -------------------------------------------------------------------------

  Revision History:

  2026/04/24 -rs:   V1.00 Initial version (for Janitza UMG96RM)

****************************************************************************/

#ifndef _UMG96RM_H_
#define _UMG96RM_H_

#include "SmartMeter.h"



/***************************************************************************/
/*                                                                         */
/*                                                                         */
/*          CLASS  UMG96RM                                                 */
/*                                                                         */
/*                                                                         */
/***************************************************************************/

class UMG96RM : public SmartMeter
{

    //-----------------------------------------------------------------------
    //  Definitions
    //-----------------------------------------------------------------------

    public:

        typedef struct
        {
            float           m_flVoltage_L1;                 // Reg 19000/19001: Voltage L1               [V]
            float           m_flVoltage_L2;                 // Reg 19002/19003: Voltage L2               [V]
            float           m_flVoltage_L3;                 // Reg 19004/19005: Voltage L3               [V]
            float           m_flCurrent_L1;                 // Reg 19012/19013: Current I L1             [A]
            float           m_flCurrent_L2;                 // Reg 19014/19015: Current I L2             [A]
            float           m_flCurrent_L3;                 // Reg 19016/19017: Current I L3             [A]
            float           m_flPower_L1;                   // Reg 19020/19021: Real power P1 L1         [W]
            float           m_flPower_L2;                   // Reg 19022/19023: Real power P2 L2         [W]
            float           m_flPower_L3;                   // Reg 19024/19025: Real power P3 L3         [W]
            float           m_flCurrent_Sum;                // Reg 19018/19019: Vector sum IN=I1+I2+I3   [A]
            float           m_flPower_Sum;                  // Reg 19026/19027: Sum Psum3=P1+P2+P3       [W]
            float           m_flFrequency;                  // Reg 19050/19051: Frequency                [Hz]
            float           m_flEnergy;                     // Reg 19068/19069: Real energy L1..L3       [kWh] (converted from Wh)

        } tMeasurements;


    private:

        const char*         DEV_TYPE_NAME       = "UMG96RM";
        static  const int   MODBUS_REG_START    = 19000;
        static  const int   MODBUS_REG_COUNT    = 70;



    //-----------------------------------------------------------------------
    //  Public Methodes
    //-----------------------------------------------------------------------

    public:

        UMG96RM();
        ~UMG96RM();

        SmartMeter::tError  Setup (const char* pszRtuInterface_p, long lBaudrate_p, int iModbusSlaveID_p, const char* pszLabel_p = NULL);
        SmartMeter::tError  ReadModbusRegs (void);
        const char*         DumpModbusRegs (void);

        void                GetMeasurements (UMG96RM::tMeasurements* pMeasurements_p);
        const char*         GetMeasurementsAsText (void);
        const char*         GetMeasurementsAsJson (time_t tmTimeStamp_p = 0, bool fAddDevInfo_p = false);

        int                 GetModbusSlaveID (void);
        const char*         GetDevTypeName (void);
        const char*         GetErrorText (SmartMeter::tError SmError_p);


    //-----------------------------------------------------------------------
    //  Private Attributes
    //-----------------------------------------------------------------------

    private:

        modbus_t*           m_pMBInst                               = NULL;
        int                 m_iModbusSlaveID                        = -1;
        uint16_t            m_aui16ModbusRegBuff[MODBUS_REG_COUNT]  = { 0 };
        char                m_szLabel[128]                          = { 0 };

        char                m_szModbusRegDump[4096]                 = { 0 };
        char                m_szMeasurementsText[1024]              = { 0 };
        char                m_szMeasurementsJson[1024]              = { 0 };



    //-----------------------------------------------------------------------
    //  Private Methodes
    //-----------------------------------------------------------------------

    private:

        void                DecodeMeasurements (UMG96RM::tMeasurements* pMeasurements_p);
        const char*         BuildTextString (void);
        const char*         BuildJsonString (time_t tmTimeStamp_p, bool fAddDevInfo_p);

};



#endif  // _UMG96RM_H_


